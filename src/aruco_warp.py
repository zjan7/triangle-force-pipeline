...
def detect_aruco(
    ...
    Works with both newer and older OpenCV ArUco APIs.
    """
...
def build_marker_corner_dict(
    ...
    marker_id -> marker corner coordinates

    Each value has shape (4, 2).
    """
...
def get_matched_points(
    ...
    """
    Get corresponding ArUco corner points between reference and deformed images.

    Returns
    -------
    reference_points:
        Corner points in the reference image.

    deformed_points:
        Corner points in the deformed image.

    used_ids:
        Marker IDs used for the homography.
    """
...
def warp_deformed_to_reference(
    reference_image_path: str | Path,
    deformed_image_path: str | Path,
    output_dir: str | Path = PROJECT_ROOT / "outputs" / "aruco_warp_test",
    aruco_type: str = "DICT_4X4_100",
    required_ids: list[int] | None = None,
    ...
    reference_debug = draw_detected_markers(
        reference_image,
        reference_corners,
        reference_ids,
    )
    deformed_debug = draw_detected_markers(
        deformed_image,
        deformed_corners,
        deformed_ids,
    )
    ...
    reference_points, deformed_points, used_ids = get_matched_points(
        reference_dict,
        deformed_dict,
        required_ids=required_ids,
        require_all=require_all_markers,
    )
    ...
    aligned_deformed = cv2.warpPerspective(
        deformed_image,
        homography,
