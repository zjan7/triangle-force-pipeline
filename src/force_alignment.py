...
def load_generated_forces(
    ...
    if force_matrix_full_path is not None and Path(force_matrix_full_path).exists():
        ...
        if force_matrix.ndim != 2 or force_matrix.shape[1] < 11:
            raise ValueError(
                f"force_matrix_full.npy should have at least 11 columns, got {force_matrix.shape}"
            )
...
def align_forces_to_opencv_order(
    ...
    generated_forces = load_generated_forces(
    ...
    # Initial transform.
    A, b = initial_physical_to_pixel_guess(
        physical_points=generated_ref_centroids_physical,
        pixel_points=opencv_ref_centroids_px,
    )

    # Refine transform and mapping.
    for _ in range(n_refinement_steps):
        generated_ref_centroids_px_est = apply_affine(
            generated_ref_centroids_physical,
            A,
            b,
        )

        opencv_to_generated, distances = one_to_one_nearest_mapping(
            generated_points_px=generated_ref_centroids_px_est,
            opencv_points_px=opencv_ref_centroids_px,
        )
        ...
        A, b = fit_affine(
            physical_points=generated_ref_centroids_physical[matched_generated_indices],
            pixel_points=opencv_ref_centroids_px[valid_opencv_rows],
        )

    # Final mapping.
    generated_ref_centroids_px_final = apply_affine(
        generated_ref_centroids_physical,
        A,
        b,
    )
    ...
    opencv_to_generated, distances = one_to_one_nearest_mapping(
        generated_points_px=generated_ref_centroids_px_final,
        opencv_points_px=opencv_ref_centroids_px,
    )
