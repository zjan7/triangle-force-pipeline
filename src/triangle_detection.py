...
# Stable IDs by grid rows and columns.
use_grid_ids = True
row_tolerance_px = 30
...
def detect_triangles(
    ...
    # Dit is jouw originele detectiefunctie.
...
def draw_displacement_arrows(image, matches, scale=1.0):
    ...
    for m in matches:
        start = (int(round(m["cx_ref"])), int(round(m["cy_ref"])))
            ...
            int(round(m["cx_ref"] + scale * m["dx_px"])),
            int(round(m["cy_ref"] + scale * m["dy_px"])),
        )
...
def run_triangle_detection(show=False):
    ...
    reference_triangles, reference_annotated, reference_binary = detect_triangles(
        reference_img,
        min_area=min_area,
        max_area=max_area,
        assign_ids=True,
    ...
    deformed_triangles, deformed_detected_annotated, deformed_binary = detect_triangles(
        deformed_img,
        min_area=min_area,
        max_area=max_area,
        assign_ids=False,
