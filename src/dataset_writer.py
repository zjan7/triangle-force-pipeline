...
def write_accepted_sample(
    ...
    triangle_matrix_full_path = get_path_from_result(
        triangle_result,
        ["triangle_matrix_full_path", "triangle_matrix_path"],
    )

    reference_corners_path = get_path_from_result(
        reference_result,
        ["reference_corners_path", "reference_zero_force_corners_path", "corners_path"],
    )

    y_forces_path = get_path_from_result(
        deformation_result,
        ["y_forces_path", "forces_path"],
    )

    force_matrix_full_path = get_path_from_result(
        deformation_result,
        ["force_matrix_full_path", "force_matrix_path"],
    )
    ...
    homography_path = get_path_from_result(
        aruco_result,
        ["homography_path"],
    )
    ...
    deformed_corners_path = get_path_from_result(
        deformation_result,
        ["deformed_corners_path", "corners_path"],
    )
    copy_file(deformed_corners_path, raw_dir / "deformed_corners.npy")

    reference_data_path = get_path_from_result(
        reference_result,
        ["reference_data_path", "reference_zero_force_data_path"],
    )
    copy_file(reference_data_path, raw_dir / "reference_zero_force_data.npy")
    copy_file(
        get_path_from_result(reference_result, ["reference_image_path", "image_path"]),
        check_images_dir / "reference_zero_force_with_aruco.png",
    )

    copy_file(
        get_path_from_result(deformation_result, ["deformed_image_path", "image_path"]),
        check_images_dir / "deformed_with_aruco.png",
    )

    copy_file(
        get_path_from_result(aruco_result, ["aligned_image_path", "aligned_deformed_path"]),
        check_images_dir / "aligned_deformed.png",
    )

    copy_file(
        get_path_from_result(aruco_result, ["overlay_path"]),
        check_images_dir / "overlay_reference_vs_aligned.png",
    )

    copy_file(
        get_path_from_result(deformation_result, ["force_plot_path"]),
        check_images_dir / "force_plot.png",
    )
    ...
    if triangle_output_dir is not None:
        ...
        for image_name in [
            ...
            copy_file(
                triangle_output_dir / image_name,
                check_images_dir / image_name,
            )

        copy_file(
            triangle_output_dir / "triangle_matrix.csv",
            full_matrices_dir / "triangle_matrix.csv",
        )

        copy_file(
            triangle_output_dir / "triangle_analysis.json",
            full_matrices_dir / "triangle_analysis.json",
        )
    metadata = {
