...
def main():
    ...
    module_area = 0.01 * 0.01  # 1 cm^2 modules
    ...
    for i in tqdm.tqdm(range(tris.n_x)):
        ...
        for j in tqdm.tqdm(range(tris.n_y), leave=False):
            ...
            ax.fill([c1[0], c2[0], c3[0]], [c1[1], c2[1], c3[1]], color="black")
    ...
    # Top marker: ID 1
    place_marker(ax, marker_paths[0], center_x=(x_min + x_max) / 2, center_y=y_max + margin + marker_size / 2, size=marker_size, label="ID 1")

    # Left marker: ID 2
    place_marker(ax, marker_paths[1], center_x=x_min - margin - marker_size / 2, center_y=(y_min + y_max) / 2, size=marker_size, label="ID 2")

    # Right marker: ID 3
    place_marker(ax, marker_paths[2], center_x=x_max + margin + marker_size / 2, center_y=(y_min + y_max) / 2, size=marker_size, label="ID 3")
