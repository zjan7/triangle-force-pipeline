...
def generate_deformation_sample(output_dir: str | Path = PROJECT_ROOT / "outputs" / "deformation_test", grid_width: float = 0.1, grid_height: float = 0.1, target_triangles: int = 200, marker_ids: list[int] | None = None, dpi: int = 300, show_plot: bool = False,) -> dict[str, Any]:
    ...
    save_force_plot(force_generator=force_generator, output_path=force_plot_path, grid_width=grid_width, grid_height=grid_height,)
    ...
    for i in tqdm.tqdm(range(tris.n_x), desc="Generating deformed triangles"):
        ...
        for j in tqdm.tqdm(range(tris.n_y), leave=False):
            ...
            c1, c2, c3 = bep.solve_module(force_vector, upside_down=upside_down,)
